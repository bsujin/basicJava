	...
	public int saldetail() {
		...
		for (int i = 0; i < detailSal.size(); i++) {
			System.out.println("\n\t ▶날짜 : " + detailSal.get(i).get("DT")
					...
					+ "\n\t ▶추가수당 : "
					+ detailSal.get(i).get("NVL(OVER_SAL,0)")
					+ "\n\t ▶총 급여 : " + detailSal.get(i).get("TOTAL_SAL")
	...
	public int totalCal() {
		...
		for (int i = 0; i < totalSal.size(); i++) {
			System.out.println("\n\t ▶날짜 : " + totalSal.get(i).get("DT")
					+ "\n\t ▶기본급 : " + totalSal.get(i).get("SAL") + "\n\t ▶초과수당 : "
					+ totalSal.get(i).get("NVL(OVER_SAL,0)") + "\n\t ▶총급여 : "
					+ totalSal.get(i).get("TOTAL_SAL") + "\n\t ▶지급일 : "
	...
	public int overCal() {
		...
		System.out.println();
			...
			System.out.println("\n\t ▶날짜 : " + overSal.get(i).get("DT")
					...
					+ "\n\t ▶초과수당 : " + overSal.get(i).get("NVL(OVER_SAL,0)"));
